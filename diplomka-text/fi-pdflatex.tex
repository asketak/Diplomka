...
\thesissetup{
 ...
 which can be helpful to deanonymize users of cryptomarkets available via Tor on the dark web. The aim of this tool is to help investigators collect intelligence about entities related to these drug markets. Users and operators of these markets employ multiple means to prevent their deanonymization. Cryptomarkets are operated as hidden services, PGP encryption is often required to communicate between multiple parties and bitcoin is used as a way of paying for goods or services.
    ...
    We created a tool collecting these data, importing them into the database, running multiple heuristics
    and providing a user with an interface for visualizing data and metadata of addresses and identities related to 
    cryptomarkets and blockchain.
    The tool can find the nearest addresses related to drug markets for given bitcoin address and also find the nearest addresses that are mentioned in scraped websites. 
    
    To test the accuracy of this tool, we created two profiles on Valhalla cryptomarket and deposited and withdrew bitcoins multiple times. This way, we obtained multiple bitcoin addresses owned by Valhalla cryptomarket.
    We ran the tool and certified the tool's ability to identify, whether the addresses we obtained from these transactions were identified as belonging to the same identity (eq. Valhalla market).
    },
...
multiple bitcoin addresses that are owned by the cryptomarket and added this information to database also.
Tool also provides a GUI, where the tool user can insert an address and the program finds addresses with linked identities
...
We also estimated the cashflow of the whole cryptomarket from the webscraped data and compared
...
We estimated the total cryptomarket's cashflow just from user feedbacks, so the total cashflow might be bigger.
The transactions and heuristics that we performed led to identifying 9\% of cryptomarkets estimated cashflow.
...
The first metric is number of active listings.
...
\url{https://direclown.files.wordpress.com/2017/07/rdarknetmarkets_censored2.png}}. We therefore consider these sites to be relevant enough not to miss
...
The goal of the Tor project is to protect privacy of its users and allow them to communicate
over internet anonymously.
Tor hides user's identity from the server he is accessing and also anyone doing network surveilance.
...
The hash of first 80 bits of public key is the service identifier,
...
\begin{table}
    ...
    Identity &\multicolumn{1}{|p{4cm}|}{\centering Associated deposits:withdrawals \\ from first account }&\multicolumn{1}{|p{4cm}|}{\centering Associated deposits:withdrawals \\ from second account}
...
They download Valhalla market webpages with listings, profile pages of vendors and feedbacks.
Then they parse downloaded webpages and generate CSV files with the data.
...
The figure \ref{listingxprice} shows distribution of prices of all listings in
 ...
 bought something from vendor. We calculated the price of listings in euros as 
...
we were able to identify 9\% of estimated cryptomarket's cashflow over given month.

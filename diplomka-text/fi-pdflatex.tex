...
========================================= V TOMTO SOUBORU ZACINAJI MOJE UPRAVY TADY
...
use multiple bitcoin addresses, both for the
bitcoin core and other bitcoin wallet software. For each Valhalla market account,
...
In order to solve this issue, we used two heuristics widely used
...
For the first account, we performed these transactions on February 8th 2018, for the second one on March 4th 2018.
...
identities received between 15. 1. and 15. 2. We scraped the Valhalla cryptomarket between 14. 2. and 15. 2.
...
We estimated in chapter \ref{stats}, that users spent at least 527730EUR during that one month period.
...
64.2 BTC the market received within that one month timeframe.
...
Each one of the scripts named scraper\_\{bitcointalk, blockchaininfo, reddit, twitter\}.py scrape webpage mentioned
...
and generate several csv files with the transaction data and bitcoin blockchain addresses required for the core application.
The script generates multiple csv files, which are later imported into neo4j database, as well as 
...
Script \emph{cluster\_identities.py} connects to database and runs the heuristics mentioned in the chapter
...
The frequency of shipping-source countries can be found in the \ref{shipcount} table.
The \ref{richvendors} table lists countries of vendors that have achieved 10000EUR(150 vendors \~ 15\% ) or more in their revenue.
...
(except for 3) earned less than 300EUR on cryptomarket over their lifetime.
...
The \ref{listingxprice} figure shows distribution of prices of all listings in
...
these listings are somehow special, except for the price.
...
can be found in the \ref{categories} table.
Market share is a revenue of category divided by total revenue from all categories, which was 527730 EUR.
...
active listings in. Most vendors sell goods only in a few categories, hovewer, vendors with 10000EUR or more in
...
Just 129 (5 \%) of buyers had bought goods worth 10000EUR in total, rounded,
...
The distribution of buyers total spendage is on the \ref{buyerxspend} figure.
...
We scraped Valhalla market once, but the data feedbacks dat were
...
The application could be extended by adding more heuristics for clustering addresses. Some of these

...
for clustering bitcoin addresses belonging to same user and also used a label propagation
algorithm \parencite{raghavan2007near} for detecting communities inside graphs.
...
Both of these heuristics are pretty strict and have just a small chance of falsely merging
addresses not belonging to one user.

Label propagation algorithm it is popular algorithm for clustering and community detection
in extremely big graphs because of its linear complexity.

We labeled each address in our blockchain transaction graph representation with unique user number
and alternately run first and second heuristics until there were no changes in the user number property of addresses.
We than ran the label propagation algorithm on the 

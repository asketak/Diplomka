...
one of the most popular drug markets available via TOR.
...
For web scraping Walhalla market,
we did not have to implement any login and captcha solving functionality,
because Valhalla listings and vendor pages are accessible
and showing same data when accessing them without being logged in.

Addresses of all market listings are in pattern
\textbf{\url{http://valhallaxmn3fydu.onion/products/xxx}} where \textbf{xxx}
is a number incrementing with each new listing.
Only 1/3 of numbers let to the valid listing page.
Rest of the numbers led to 404 error. We believe,
that these numbers refer to listings that were disabled by the vendor or administrator.

Vendor profile pages were in format \textbf{\url{http://valhallaxmn3fydu.onion/xxx}}
and their reviews in the format
 \textbf{\url{http://valhallaxmn3fydu.onion/xxx/palautteet}} where \textbf{xxx} is vendor username.
...
which we used for downloading and scraped the vendor's profile and feedback pages. 
...
We downloaded hundreds of pictures both from Valhalla.

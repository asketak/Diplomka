...
The first metric is its size. Walhalla market is well known operating cryptomarket.
...
the characteristics of vendors from both markets, focusing on the difference
 ...
 This work focus on describing the vendors from currently operating drug market Valhalla 
 and see, if the behaviour of vendors or the nature of cryptomarkets
 has significantly changed. We also not only passively scraped cryptomarket's webpage,
...
The data was collected from Valhalla drug market on 20.1.2018.
...
which can not be used by wget. So we created an HTTP proxy by privoxy, which redirected the traffic through
TOR SOCKS proxy.

For web scraping Walhalla market, we did not have to implement any login and captcha solving functionality,
because Valhalla listings and vendor pages are accessible and showing same data when accessing them without being logged in.

Addresses of all market listings are in pattern http://valhallaxmn3fydu.onion/products/xxx where xxx is number incrementing with each new listing. Only 1/3 of numbers let to the valid listing page.
Rest of the numbers led to 404 error. We believe, that these numbers refer to listings that were disabled by the vendor
 or administrator.
...
Vendor profile pages were in format http://valhallaxmn3fydu.onion/xxx and their reviews in the format
 http://valhallaxmn3fydu.onion/xxx/palautteet where xxx is vendor username.
 We were able to scrape profile and feedback page of each vendor who had active listing at the time of scraping. 
...
After downloading all the listing pages, we parsed the downloaded files using python and common Linux command line tools(cat, grep, cut, sed). We have not used python HTML parsing libraries( like beautifulsoup) for parsing downloaded
webpages because HTML elements of Valhalla web pages don't have any unique identifiers and so these libraries bring
 us no advantage.
 
By this, we got 666 unique vendors name, so we downloaded and scraped the vendor's profiles pages from the Valhalla 
Valhalla market in a similar way.
...
However, we managed to download 20000 listings out of 100000.
...
We downloaded hundreds of pictures both from Valhalla and dream market.
Only metadata directly depending on image content(like the amount of red, green and blue colours) differ,
...
We created vendor account in both markets and uploaded an image with custom-made metadata to see,
if the metadata were scraped and the same version of software version appears. It happened so for both markets,
...
We tried to scan ports of drug markets servers and fingerprint their web server, in order to find any vectors of further 
information gathering.
We scanned both drug markets servers using netcat, finding, that the only opened port is number 443(HTTPS), which is use
d by web server.
We used httprecon to fingerprint used HTTP server. The fingerprinting consists of sending multiple malformed HTTP requests and comparing the web server output with the database of responses by different web servers.
The results of fingerprinting can be see in figure xxx, the best matches are various modern versions of apache web server
.
The results of port scan and web server printing doesn't indicate any way how to gather data about drug markets servers.
...
Than marking the addresses from where the bitcoins were received as 

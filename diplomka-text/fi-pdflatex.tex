...
The site \emph{https://www.deepdotweb.com} has Alexa ranking 23500, which roughly corresponds to thousands of visitors a day.
...
When someone pays by bitcoins, the recipient can see the recipient address
...
bitcoin address that owns it and a script that describes how the input/output can be used.
...
spend just a portion of bitcoins from the output. In order to divide bitcoins into smaller parts,
...
While the Tor usage itself can be detected by a third party, the third party can not decrypt user's data that are transmitted via Tor. Some websites restrict access from Tor network due to many risks involved.
...
Each one of the scripts scrapes webpage mentioned

...
bitcoin transaction with Valhalla market account.1
...
We had two accounts on Valhalla market, each performing 1 deposit and immidiatelly 10 withdrawals.
First account did that on 15.1.2018, the second one on 12.2.2018.
We created new bitcoin wallet via mycelium mobile app, deposited bitcoins to it
from bittrex exchange and used only that wallet for first account.
The second bitcoin wallet was created by coinomi mobile app and 
...
We check the addresses we identified by our deposits and withdrawals,
if they were clustered as one identity. The diagram \ref{clustertest} shows, how the 
addresses were associated to common identities. Each collor represents one identity, circles
are our transaction and rectangles Valhalla market addresses.

We looked at the identities mentioned in previous paragraph 
and checked, how much bitcoins have these
identities recieved between 15.1 and 15.2. We scraped Valhalla cryptomarket between 13.2 and 15.2
...
We estimated in chapter \ref{stats}, that users spent 527730EUR
With average bitcoin price 8219EUR it roughly equals 527730/8219 = 64.2BTC
The amount of bitcions identities from previous paragraph recieved is in table \ref{clustertest2}.

Our heuristics were not enough to cluster majority of market's transactions/addresses as one identity.
Hovewer, with just two accounts, 2 deposits and 20 withdrawals, we were able to identify 7.3BTC out of estimated
64.2 BTC the market recieved.
...
The created application is fully working and production ready, it 
...
different options viable for storing and retrieving data on better hardware(more RAM) could be considered and tested.

...
"""
    ...
    tabledata_rec =  """MATCH (start:Identity)-[]-(strt:Address )<-[:USES]-(o1:Output)
    ...
    tabledata_send =  """MATCH (end:Address {address: '""" + address+ """'} )<-[:USES]-(o1:Output)
    ...
    total_recieved =  """MATCH (a:Address {address: '""" + address + """' })<-[:USES]-(o),
          (o)-[r:INPUT|OUTPUT]-(t)
        WITH a, t,
    ...
    total_sent =  """MATCH (a:Address {address: '""" + address + """' })<-[:USES]-(o)
            WHERE (o)-[:INPUT]->()
            RETURN sum(o.value)"""

    distancedata_req =  """MATCH (end:Address {address: '""" + address+ """'} )<-[:USES]-(o1:Output)
    -[:INPUT|OUTPUT*1..6]->(o2:Output)-[:USES]->(strt:Address)-[]-(start:Identity),
    p = shortestpath((o1)-[:INPUT|OUTPUT*1..6]->(o2))
    with start,strt,p
    limit 1
    ...
    r = requests.post(' http://localhost:7474/db/data/transaction/commit',
     json={ "statements" : [ { "statement" : tabledata_send }
    , { "statement" : tabledata_rec }
    , { "statement" : total_recieved }
    , { "statement" : total_sent }
    , { "statement" : distancedata_req }
     ]
      })
